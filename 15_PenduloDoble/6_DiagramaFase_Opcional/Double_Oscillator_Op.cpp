...
int main()
{
    ...
    //  Theta 1
    double Theta1Inicial = -PI;
    double Theta1Final = PI;
    double Theta1Puntos = 150;
    double Theta1h = (Theta1Final-Theta1Inicial)/(Theta1Puntos-1);
    double Omega;   // Omega cuadrado
    double l;
    //  Theta 2
    double Theta2Inicial = -PI;
    double Theta2Final = PI;
    double Theta2Puntos = 150;
    double Theta2h = (Theta2Final-Theta2Inicial)/(Theta2Puntos-1);
    
    for(kk= 0; kk<Theta1Puntos; kk++)
    {
        
        cout<<"\n Theta 1: "<<kk*Theta1h+Theta1Inicial;

        for(k=0; k<Theta2Puntos; k++)
        {
            Omega = pow(10,5);  //  Ver ejercicio 1
            l = g/Omega;
        
            // Valores iniciales:
            Vec[0] = kk*Theta1h+Theta1Inicial;        // Theta 1
            Vec[1] = k*Theta2h+Theta2Inicial;                        // Theta 2
            Vec[2] = 0;                             // P 1
            Vec[3] = 0;                             // P 1
