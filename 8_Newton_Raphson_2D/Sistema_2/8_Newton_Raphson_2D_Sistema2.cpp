...
int main()
{
    ...
    x0 = -6;
    y0 = 7;
    myfile.open ("NewtonRaphson2D_x0_-6_y0_7.txt");
...
double* GaussJordan(double vector[2], double deltaX[2])
{
    ...
    J[0][0] = y*sin(x)+0.6*pow(x-0.5,2);
    J[0][1] = 2*y-cos(x);
    J[0][2] = -(y*y-y*cos(x)+0.2*pow(x-0.5,3));
    
    J[1][0] = 2*cos(y-5)-2*(x+3);
    J[1][1] = -2*x*sin(y-5);
    J[1][2] = -(2*x*cos(y-5)-pow(x+3,2));
 /*
    J[0][0] = 3;
    J[0][1] = 1;
    J[0][2] = 22;
...
double* Funcion(double vector[2], double Fun[2])
{
    ...
    Fun[0] = y*y-y*cos(x)+0.2*pow(x-0.5,3);
    Fun[1] = 2*x*cos(y-5)-pow(x+3,2);
